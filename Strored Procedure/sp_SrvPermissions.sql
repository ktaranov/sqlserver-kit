...
IF LEN(ISNULL(@Principal,@Role)) > 0
    IF @Print = 1
        SET @sql = @sql + NCHAR(13) + N'  AND Logins.name LIKE ' + ISNULL(+QUOTENAME(@Principal,''''),QUOTENAME(@Role,''''))
    ELSE
        SET @sql = @sql + NCHAR(13) + N'  AND Logins.name LIKE ISNULL(@Principal,@Role) '
    
IF LEN(@Type) = 1
    IF @Print = 1
        SET @sql = @sql + NCHAR(13) + N'  AND Logins.type = ' + QUOTENAME(@Type,'''')
    ELSE
...
ELSE
    EXEC sp_executesql @sql, N'@Principal sysname, @Role sysname, @Type char(1)', @Principal, @Role, @Type
...
IF LEN(ISNULL(@Principal,'')) > 0
    IF @Print = 1
        SET @sql = @sql + NCHAR(13) + N'  AND Logins.name LIKE '+QUOTENAME(@Principal,'''')
    ELSE
        SET @sql = @sql + NCHAR(13) + N'  AND Logins.name LIKE @Principal'
