...
DECLARE @exec_query_memory_grants TABLE (
	session_id			int NOT NULL,
	request_id			int NOT NULL,
	requested_mb		numeric(12, 3) NOT NULL,
	granted_mb			numeric(12, 3) NULL,
	minimum_required_mb	numeric(12, 3) NOT NULL,
	used_mb				numeric(12, 3) NOT NULL,
	max_used_mb			numeric(12, 3) NOT NULL,
	ideal_mb			numeric(12, 3) NOT NULL,
	est_query_cost		numeric(12, 1) NOT NULL,
...
SELECT session_id, request_id,
	...
	1.0*used_memory_kb/1024 AS used_mb,
	1.0*max_used_memory_kb/1024 AS max_used_mb,
	...
	SELECT name,
		...
		'USE ['+[name]+'];
		 SELECT '+CAST(database_id AS varchar(max))+', f.[file_id], f.type_desc AS filetype,
			...
			NULL AS total_bytes, NULL AS available_bytes, NULL AS file_system_type' END)+', NULL, NULL
		 ...
		 SELECT '+CAST(database_id AS varchar(max))+', -1 AS [file_id], ''IN-MEMORY'' AS filetype,
			...
			NULL AS total_bytes, NULL AS available_bytes, NULL AS file_system_type, NULL, NULL
...
UPDATE f SET f.primary_replica=ags.primary_replica, f.[availability_mode]=ar.[availability_mode]
FROM @files AS f
...
--	ISNULL(NULLIF(xc.client_net_address, xc.local_net_address), 'local')+ISNULL(':'+CAST(xc.client_tcp_port AS varchar(max)), '') AS [Client addr],
	...
	ISNULL(CAST(NULLIF((SELECT COUNT(DISTINCT wt.exec_context_id)
		FROM @os_waiting_tasks AS wt
		WHERE wt.session_id=xc.session_id), 0) AS varchar(10)), '') AS Threads,
